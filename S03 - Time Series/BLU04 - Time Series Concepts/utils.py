...
def measure_the_earth(std, n=1, verbose=False):
    
    corect_measure=6371
    rng = np.random.default_rng()
    measurement = rng.normal(corect_measure, corect_measure * std, n) 
    
    if verbose: 
        measure_error(np.mean(measurement), corect_measure=corect_measure)
...
def plot_number_of_tries(series):
    ...
    plt.axhline(6371, color='g', ls=':')
    plt.ylim([series.min()-0.1*(series.max()-series.min()), series.max()+0.1*(series.max()-series.min())])
    plt.title('Expanding window of the measures of the radius of the Earth (true value in green)')
