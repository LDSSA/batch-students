...
def fit_line(x, y):
    ...
    return lr.predict(x.values.reshape(-1, 1))  

    
def fit_lin_reg(df, ax=None):
    if ax==None:
        fig, ax = plt.subplots()
    df.plot(kind='scatter', x='age', y='minutes_per_day', ax=ax, alpha=.5);
    y_plot = fit_line(df['age'], df['minutes_per_day'])
    ax.plot(df['age'], y_plot, ls=':', c='r', label='prediction')
...
def fit_complex_model(df, ax=None):
    if ax==None:
        fig, ax = plt.subplots()
    df.plot(kind='scatter', x='age', y='minutes_per_day', ax=ax, alpha=.5);
    y_plot = fit_knn(df['age'], df['minutes_per_day'])
    ax.plot(df['age'], y_plot, ls=':', c='r', label='prediction')
    ax.set_title('Complex model fit')
    ax.legend();


def generate_time_on_tv(random_seed=100, outlier_1=130, outlier_2=50):
    ...
    df.loc[35, 'minutes_per_day'] = outlier_1
    df.loc[70, 'minutes_per_day'] = outlier_2
...
def plot_super_flexible(X, y):
    ...
    knn_k1.fit(X.to_numpy(), y.to_numpy())
    preds = knn_k1.predict(X.to_numpy())
    plot_decision_regions(X.to_numpy(), y.to_numpy(), clf=knn_k1, legend=2)
    plt.title("KNN (k=1)")
    plt.xlabel('Color')
    plt.ylabel('IBU');
    return preds
...
def plot_just_right(X, y):
    ...
    knn_k9.fit(X.to_numpy(), y.to_numpy())
    preds = knn_k9.predict(X.to_numpy())
    plot_decision_regions(X.to_numpy(), y.to_numpy(), clf=knn_k9, legend=2)
    plt.title("KNN (k=9)")
    plt.xlabel('Color')
    plt.ylabel('IBU');
    return preds
